...
main (int argv, char *argc[])
{
  ...
  sc_icounterInit ();
  //                 0=======-------
  sc_memorySet (0, 0b000100000001010);
  sc_memorySet (1, 0b000100000001010);
  sc_memorySet (2, 0b000100000001010);
  sc_memorySet (3, 0b000100000001010);
  // загружаем ячейку 10 в аккумулятор
  sc_memorySet (4, 0b001100000001011);
  // складываем значение аккумулятора с 11 ячейкой
  sc_memorySet (5, 0b001100010001100);
  // вычитаем из аккумулятора с 12 ячейку
  sc_memorySet (6, 0b001000010001101);
  // выгружаем из аккумулятора значение в 13 ячейку
  sc_memorySet (7, 0b011000000001101);
  // логический двоичный сдвиг влево ячейки 13 [CHL]
  sc_memorySet (8, 0b001000010001110);
  // выгружаем из аккумулятора значение в 14 ячейку
  sc_memorySet (9, 0b010101010001101);
